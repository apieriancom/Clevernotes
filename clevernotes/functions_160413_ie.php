...
//test if running off live server
add_action('init', 'is_not_local');
...
//if (is_page_template('profile-page.php') ) {
	function jquery_method() {		
		...
		wp_enqueue_script('jquery-ui-datepicker',  FILE_URL . 'jquery.ui.datepicker.min.js', array('jquery','jquery-ui-core') );
...
function competition_form_sc( $atts ) {	                     
	...
	$output = "<form action='/competition-thanks' method='post' id='competition_form'>
			...
			<fieldset>
				...
				<div style='margin-top: 10px;'>
				<label for='mobile' class='bold'>Mobile Number</label><input class='competition_field' type='text' name='mobile' value='".$cnt->MobilePhone."'>
				</div>
				<div style='margin-top: 10px;'>
...
//if (is_page_template('profile-page.php') ) {
	function jquery_leanmodal() {		
		...
		if ( is_page('membership') || is_page('new-buy-flow') || is_front_page() || is_page('oral-crash-course')  || is_page('oral-crash-course-landing')) {
