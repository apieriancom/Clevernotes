...
function sfLogin($loginInfo){
  ...
  try{
		...
		if($result->result->bNewUser)header("Location: https://".$_SERVER['HTTP_HOST']."/oral-crash-course/#buynow");
		// Else send them to the welcome page
		else header("Location: https://".$_SERVER['HTTP_HOST']."/oral-crash-course/#buynow");	
